    ...
    <section className="py-24 bg-white">
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="mb-16">
          ...
          <p className="text-navy/60 mt-6 max-w-2xl">
        ...
        <div className="space-y-8">
          {POLICIES.map((p, i) => (
            <div key={i} className="flex gap-6 p-6 border border-navy/5 rounded-2xl bg-cream/30">
              <div className="text-gold mt-1 shrink-0">{p.icon}</div>
              <div>
                <h4 className="text-lg font-bold text-navy mb-2">{p.title}</h4>
                <p className="text-navy/70 leading-relaxed text-sm md:text-base">{p.desc}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Contact Footer */}
        <div className="mt-16 p-8 bg-navy text-white rounded-2xl text-center">
          <h4 className="font-serif text-xl mb-4">Have Questions?</h4>
          <p className="text-white/70 mb-6">Need clarification on our policies? We're here to help.</p>
          <a href="https://wa.me/918920924077" className="inline-block bg-gold text-navy px-8 py-3 rounded-full font-bold hover:scale-105 transition-transform">
